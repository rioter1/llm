...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # This is a common technique to scale the initialization
        # based on the number of layers, 
        # which can help stabilize training.projection layers in transformers) 
        # may require a different scale of initialization to ensure 
        # effective learning.
    ...
    def forward(self, idx, targets=None):
        ...
        # self.transformer was a moduledict object and the keys can be called as below
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        sd = model.state_dict()
        sd_keys = sd.keys()
        # the pre-trained attention bias is not loaded, allowing the 
        # model to learn its own attention bias during fine-tuning or training from scratch.
        # Dropping these keys ensures that there is no mismatch between the 
        # pre-trained values and the model's expected initialization 
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        ...
        '''
        Weight decay is a commonly used regularization technique that helps prevent 
        overfitting by adding a penalty term to the loss function. 
        It encourages the model to learn smaller weights, which can lead to better generalization
        
        The rationale behind this separation is that weight decay is typically applied
        to the weight tensors (e.g., in fully connected layers, 
        convolutional layers, or embeddings) to encourage sparsity and 
        prevent overfitting. However, it is often not applied to biases and layer norms, 
        as they have different properties and may not benefit from weight decay in the 
        same way.
        '''
    ...
    # since i have an rtx 3060 gpu whereas karapathy has an A100 gpu, 
    # i am modifying the flop estimation code to work with for 3060 instaed

    def estimate_mfu(self, fwdbwd_per_iter, dt):
        """ estimate model flops utilization (MFU) in units of RTX 3060 FP16 peak FLOPS """
        ...
        # express our flops throughput as ratio of RTX 3060 FP16 peak flops
        flops_achieved = flops_per_iter * (1.0 / dt)  # per second
        flops_promised = 60e12  # RTX 3060 FP16 peak flops is 60 TFLOPS
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            '''
            In PyTorch, when you create a model as a subclass of nn.Module, 
            you can call the model instance directly like a function. 
            This is equivalent to calling the forward method of the model.

            The self(idx_cond) call passes idx_cond (the input tensor) to the model, 
            which processes it and returns the logits (and possibly other outputs

            The line logits, _ = self(idx_cond) calls the model's forward method, 
            which is standard in PyTorch.
            '''
